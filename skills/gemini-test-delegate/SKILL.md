...
[Gemini CLI 위임 프롬프트 생성]
        ↓
[gemini --yolo -m gemini-2.5-pro -p "..." 명령어 출력]
        ↓
[테스트 파일 생성 및 검증]
```
...
아래 템플릿을 채워 `gemini --yolo -m gemini-2.5-pro -p "..."` 명령어를 생성합니다.
...
> **공통 플래그**
> - `--yolo`: 툴 실행 자동 승인 및 풀 툴셋 활성화 (`write_file`, `run_shell_command` 등)
> - `-m gemini-2.5-pro`: 안정적인 용량의 모델 고정 (`gemini-X.X-pro-preview` 계열 용량 부족 회피)

### 방법 A: 직접 파이프 (구현 파일을 컨텍스트로 전달)

```bash
cat {구현_파일_경로} | gemini --yolo -m gemini-2.5-pro -p "{생성된_프롬프트}"
```

### 방법 B: 파일 참조 플래그 사용

```bash
gemini --yolo -m gemini-2.5-pro -p "{생성된_프롬프트}" --file {구현_파일_경로}
```
...
gemini --yolo -m gemini-2.5-pro -p "$(cat /tmp/test-delegate-prompt.txt)"
```
...
- **`--yolo` 플래그**: non-interactive(`-p`) 모드에서 `write_file`, `run_shell_command` 등 툴이 차단되는 문제를 해결. 반드시 포함할 것
- **`-m gemini-2.5-pro` 플래그**: `gemini-X.X-pro-preview` 계열은 용량 부족(429)이 잦으므로 안정 모델 고정
