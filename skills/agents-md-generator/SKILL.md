...
Q-TE1. 구현 완료 후 테스트 코드 작성을 누가 담당할까요?
  - 에이전트 직접 작성  — 에이전트가 Vitest 테스트를 스스로 작성
  - Gemini CLI 위임     — gemini-test-delegate 스킬을 통해 Gemini가 교차 검증용 테스트를 작성
                          (Gemini CLI 설치 필요: npm install -g @google/gemini-cli)
```

Q-TE1 선택 결과는 생성되는 `RULES.md#testing` 섹션과 `AGENT WORKFLOW`의 검증 3단계에 반영된다.
...
Q-TE1 선택 결과는 패키지 AGENTS.md의 `AGENT WORKFLOW` 검증 3단계에 반영된다.
...
| 구현 완료 후 테스트 작성 방식이 Gemini CLI 위임으로 설정된 경우 (RULES.md#testing 기준) | `gemini-test-delegate` |
