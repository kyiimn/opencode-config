...
### Spec-Driven Verification (명세 주도 검증)

테스트 케이스 시나리오는 **계획 단계(AGENT WORKFLOW 1단계)** 에서
사용자와 함께 정의·승인된다. 테스트 코드는 이 승인된 시나리오를
코드로 변환하는 것이며, 구현 코드를 보고 테스트를 역산하는 것을 금지한다.

```
계획 단계 (Sisyphus)          구현 단계 (Hephaestus)
┌──────────────────┐          ┌──────────────────────┐
│ 구현 명세 작성   │          │ 명세 기반 코드 구현   │
│       +          │ ──────▶  │       +               │
│ 시나리오 정의    │          │ 시나리오 → 테스트 코드│
│ (코드 없이)      │          │ (구현 후 시그니처 반영)│
│       +          │          └──────────┬───────────┘
...
**핵심 원칙**
- 시나리오 없는 테스트 케이스 추가 금지
- 시나리오와 다른 기대값으로 작성 금지
- vitest 실패 = 구현이 승인된 명세를 벗어난 것으로 판단

### 테스트 작성 기준
- 시나리오에 명시된 케이스를 모두 커버한다
- 테스트명: `{대상}_{조건}_{기대결과}`
  - 예: `getUser_존재하지않는id_NotFoundError반환`
- 시나리오 번호를 주석으로 남겨 추적 가능하게 한다
  ```typescript
...
#### 직접 작성 모드
에이전트가 승인된 시나리오를 Vitest 코드로 직접 변환한다.
`AGENT WORKFLOW` 3단계(테스트 코드 작성)에서 시나리오 테이블을 그대로 코드화하고,
`pnpm vitest run {파일}` 으로 통과를 확인한 뒤 4단계(검증)로 진행한다.
<!-- [/Q-TE1 = 에이전트 직접 작성] -->
