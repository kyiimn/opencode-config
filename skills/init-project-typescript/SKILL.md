...
description: >
  ...
  AGENTS.md / RULES.md 인터뷰 기반 생성, Sisyphus-Prometheus-Hephaestus 하네스 엔지니어링 워크플로
  설정, SDV(스펙 주도 검증) 구현 체계 수립, 그리고 그린필드 프로젝트 초기화(사용자 승인 후)까지
  한 번에 처리합니다. "프로젝트 시작", "agents.md 만들어줘", "opencode 설정", "프로젝트 규칙 파일 생성",
  "AGENTS.md 생성", "코딩 컨벤션 문서화", "프로젝트 초기화", "SDV 설정", "하네스 엔지니어링 설정",
  "oh-my-opencode 워크플로" 등의 요청에 반드시 이 스킬을 사용하세요.
---

# init-project-typescript

opencode + oh-my-opencode 환경에서 **AGENTS.md 작성 → 하네스 엔지니어링·SDV 체계 수립 → 그린필드 초기화**를
...
### 하네스 엔지니어링 워크플로
생성되는 AGENTS.md에는 oh-my-opencode의 에이전트 역할 분담이 명시됩니다.

| 에이전트 | 역할 |
|----------|------|
| **Sisyphus** | 태스크 분해·위임·조율. 승인 게이트 실행 |
| **Prometheus** | 계획 문서 작성 (구현 명세 + 테스트 케이스 시나리오). `/start-work` 전 단계 |
| **Hephaestus** | `/start-work` 이후 코드 구현·검증·리팩터링 전담 |
| **Librarian** | 코드베이스 탐색·문서 조회 |

> **표준 흐름**: Prometheus 기획 → `ask_user_input_v0` 승인 → `/start-work` → Hephaestus 구현

### SDV (Spec-Driven Verification, 스펙 주도 검증)
구현 전 테스트 케이스 시나리오를 계획 단계에서 승인받고, 승인된 시나리오만을 기준으로
테스트 코드를 작성하는 검증 체계입니다. 생성되는 `RULES.md#testing`에 SDV 원칙이 명시됩니다.

```
시나리오 정의(Prometheus) → 사용자 승인 → 코드 구현(Hephaestus)
→ 승인된 시나리오 → .test.ts 변환 → tsc → eslint → vitest
```
...
Q-TE1. 승인된 테스트 케이스 시나리오 기반으로 테스트 코드를 누가 작성할까요?
        (테스트 케이스 시나리오 자체는 계획 단계에서 항상 정의·승인됩니다)
  - 에이전트 직접 작성  — 에이전트가 승인된 시나리오를 Vitest 코드로 직접 변환
  - Gemini CLI 위임     — gemini-test-delegate 스킬을 통해 Gemini가 시나리오 기반 테스트를 작성
...
Q-TE1 선택 결과는 생성되는 `RULES.md#testing` 섹션과 `AGENT WORKFLOW`의 3단계(테스트 코드 작성)에 반영된다.
...
**테스트 작성 방식 질문 (루트 RULES.md 존재 여부와 무관하게 항상 질문)**

루트 `RULES.md`의 `#testing`에 이미 테스트 작성 방식이 명시된 경우 해당 값을 상속하며 이 질문을 건너뛴다.
명시되지 않은 경우에는 반드시 수집한다.
...
Q-TE1 선택 결과는 패키지 AGENTS.md의 `AGENT WORKFLOW` 3단계(테스트 코드 작성)에 반영된다.
...
다음 단계:
  ...
  - oh-my-opencode 기능 구현 시: Prometheus 기획 → 승인 → /start-work
```
...
다음 단계:
  - oh-my-opencode에서 기능 구현 시: Prometheus가 기획 → 승인 → /start-work
...
| 구현 완료 후 테스트 코드 작성 방식이 Gemini CLI 위임으로 설정된 경우 (RULES.md#testing 기준) | `gemini-test-delegate` |
