---
description: Scaffold a TypeScript Express API (DDD + Prisma) at the given directory. Detects monorepo context automatically — inherits CODE STANDARDS from root if found, otherwise interviews first.
---

Scaffold a TypeScript Express API (DDD + Prisma) at `$ARGUMENTS`.
...
Traverse parent directories up to git root. Check for `AGENTS.md` + `RULES.md` at root.

- Found → **monorepo mode**: read `RULES.md#code-standards`, skip PHASE 1.
...
### `src/common/infrastructure/http/ip-filter.middleware.ts` (Q-A3 = include)
...
- In monorepo mode: `tsconfig.json` must extend `@{scope}/core/tsconfig/app.json`; do not duplicate fields.
