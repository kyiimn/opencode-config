...
> `src/lib/logger.ts` is generated by the `ts-logger` skill.
> Immediately after creating the PHASE 4 files, load the `ts-logger` skill and run it in standalone mode.
