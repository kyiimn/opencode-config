...
## PHASE 5: Update AGENTS.md — Behavioral Protocol Injection

> `TodoWrite`: `update-agents-md` → `in_progress`

After `SOURCEMAP.md` is written, inject **behavioral rules** into `AGENTS.md` so that
agents actually change their scanning behavior — not just know the file exists.
...
If found, read the file and check whether a `SOURCEMAP.md` section or
`Codebase Orientation Protocol` section already exists.

- **Already exists** → verify the content matches the block below. Update if outdated.
- **Does not exist** → locate the most appropriate insertion point
  (a section listing project files, references, or documentation artifacts)
  and insert the following block.

```markdown
## Codebase Orientation Protocol (SOURCEMAP)

`SOURCEMAP.md` contains a pre-scanned project overview: directory structure,
module exports, key types, dependency graph, and architectural patterns.

### Mandatory rules — all agents MUST follow

1. **Read `SOURCEMAP.md` FIRST** before any codebase exploration.
   This is the orientation step. Do it at the start of every new task.
2. **Do NOT glob or grep the entire codebase for orientation.**
   The sourcemap already provides directory structure, module responsibilities,
   and dependency relationships. Whole-project scans for orientation are prohibited.
3. **Read individual source files ONLY when:**
   - You need to modify that specific file.
   - You need exact implementation details (function body, logic branch)
     not covered by the sourcemap.
   - The sourcemap marks a module as incomplete or outdated.
4. **Use the dependency graph** in `SOURCEMAP.md` to identify affected modules
   before making cross-cutting changes — do not grep for import statements.
5. **If `SOURCEMAP.md` does not exist or its timestamp is older than 7 days,**
   run `/gen-sourcemap` before starting work.

### What SOURCEMAP.md covers (no need to re-scan)

| Information | Section in SOURCEMAP.md |
|-------------|------------------------|
| Project name, framework, entry points | Project Overview |
| Directory tree with file counts | Directory Structure |
| Every file's exports and purpose | Module Map |
| Important types, interfaces, enums | Key Types |
| Internal module dependency graph | Dependency Graph |
| DDD layers, patterns, conventions | Architectural Patterns |
| Environment variables and config | Configuration |

### What still requires reading source files

- Exact function implementations and logic branches
- Line-level debugging and error tracing
- Test file contents (not mapped in SOURCEMAP.md)
- Runtime behavior that cannot be inferred from signatures
```

Edit with a minimal, targeted insertion — do not rewrite or reformat any existing content.
...
If found, check whether a sourcemap reference for this package already exists.

- **Already exists** → skip.
- **Does not exist** → add the following under the appropriate package listing section:

```markdown
- `{relative-path-to-package}/SOURCEMAP.md` — Source map for {package-name}.
  Agents working in this package MUST read this file before scanning source files.
  See `{relative-path-to-package}/AGENTS.md` → "Codebase Orientation Protocol" for rules.
```
