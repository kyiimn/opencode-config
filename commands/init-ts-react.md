...
description: >
  ...
  Usage: /init-ts-react <target-directory>
---
...
Detect monorepo by searching ancestors for `pnpm-workspace.yaml` or yarn workspaces.  
...
Save to `DIR/AGENTS.md`. Target ≤ 300 tokens.
...
Start: `## TRACE [{ISO}] {title} — Loop 0`
Loop: `## TRACE [{ts}] Loop {N} · Phase: {plan|implement|test|validate} · {PASS|FAIL}`
Auto-stop after 3 consecutive failures.

## FILE REFERENCES

[monorepo only] Load root RULES.md before any task. Load only the needed anchor.
| Task | Anchor |
|------|--------|
| Write code | `{root}/RULES.md#code-standards` |
| Frontend work | `{root}/RULES.md#frontend` |
```
...
> Sections: [#code-standards] · [#frontend]
...
**Font (Pretendard)**
`index.html`: `<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/static/pretendard.min.css" />`
...
devDependencies: `typescript ^5` · `vite ^6` · `@vitejs/plugin-react ^4` · `vitest ^3` · `@vitest/coverage-v8 ^3` · `jsdom ^26` · `@testing-library/react ^16` · `@testing-library/user-event ^14` · `eslint ^9` · `typescript-eslint ^8` · `@types/react ^19` · `@types/react-dom ^19` · `tailwindcss ^3` · `autoprefixer ^10` · `postcss ^8` · `clsx ^2` · `tailwind-merge ^2` · `lucide-react` [+ `@tanstack/router-plugin ^1` if ROUTER=tanstack]

**`tsconfig.json`** — target ES2022 · strict · moduleResolution Bundler · paths `"@/*": ["./src/*"]`

**`vite.config.ts`**

- plugins: `@vitejs/plugin-react` [+ `TanStackRouterVite({ routesDirectory: './src/routes' })` if ROUTER=tanstack]
- resolve alias: `@/ → src/`
...
**`index.html`** — `lang="ko"` · Pretendard `<link>` in `<head>` · `<div id="root">` · `src/main.tsx`
...
**`eslint.config.js`** — typescript-eslint recommended · no-explicit-any error · explicit-function-return-type warn
